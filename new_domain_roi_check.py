...
def check_roi(path, want_domain, images, anns, meta):
    """
        Run the viability battery on one downloaded ROI.

        path (Path): the ROI TIFF on disk; its name is looked up in the annotations.
        want_domain (str): the tumour type the ROI is expected to belong to.
        ...
        meta (pd.DataFrame): slide metadata from dataset.load_slide_metadata.

        Returns tuple: (row dict keyed by FIELDS, list of (check name, bool) pairs).
    """
    path = Path(path); fn = path.name; nnn = path.stem
    checks, notes = [], []
    ...
    # check 6 -- full annotation frame so look-alike exclusions are visible. The invariant
    # raises on a mitotic exclusion; record that as a failed check rather than an exception.
    ...
    try:
        ...
        res = {"passed": False, "n_excluded": -1, "n_excluded_strict": -1, "excluded_ann_ids": ""}
        notes.append(str(e))
    c6 = bool(res["passed"])
...
if __name__ == "__main__":
    ...
    for nnn, want_domain in CANDS:
        ...
        try:
            row, checks = check_roi(path, want_domain, images, anns, meta)
